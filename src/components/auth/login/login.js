...
const Login = () => {
    ...
    const signIn = async (userEmail, userPassword) => {
        ...
        try {
          const resData = await sendRequest(url, 'POST', JSON.stringify(credentials), {'Content-Type':'application/json'})
        if(resData) {
    ...
    return <div className='centered'>
      ...
      <br/>
      { isLoading? <CircularProgress/> : <Button variant="contained" color="primary" fullWidth type='submit' onClick={()=>signIn(email,password)}>SUBMIT</Button> }
      <br/>
      <br/>
