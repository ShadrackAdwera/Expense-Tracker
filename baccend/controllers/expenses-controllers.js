...
let DUMMY_EXPENSES = [
    {
        ...
        description: 'Bought Nyama, Ngwashe, Ndizi, Unga, Mboga, Nyanya, Kitunguu, Pili Pili',
        price: 1000,
...
const addExpense = (req,res,next) => {
    const { name, description, price, receipt } = req.body
...
const getExpenseById = (req,res,next) => {
    ...
    const foundExpense = DUMMY_EXPENSES.find(expense=>expense.id===expenseId)
    if(!foundExpense) {
...
const updateExpense = (req,res,next) => {
    ...
    const foundExpense = DUMMY_EXPENSES.find(expense=>expense.id===expenseId)
    const expenseIndex = DUMMY_EXPENSES.findIndex(expense=>expense.id===expenseId)
    if(!foundExpense) {
        return next(new HttpError('Expense does not exist',404))
    }
